...
#include "compute_miao_polarizability.h"
...
// compute COMPUTE_ID GROUP miao/polarizability model.pt cutoff prec
ComputeMiaoPolarizability::ComputeMiaoPolarizability(LAMMPS *lmp, int narg, char **arg) :
    Compute(lmp, narg, arg)
{
    if (narg != 6) 
        error->all(FLERR,"Illegal compute miao/polarizability command");
    if (igroup) 
    ...
    scalar_flag = 0;
    vector_flag = 1;
    size_vector = 6;     // xx, yy, zz, yz, zx, xy
    extvector = 1;
...
ComputeMiaoPolarizability::~ComputeMiaoPolarizability()
{
...
void ComputeMiaoPolarizability::init()
{
...
void ComputeMiaoPolarizability::init_list(int /*id*/, NeighList *ptr)
{
    list = ptr;
}


void ComputeMiaoPolarizability::compute_vector()
{
    double **x = atom->x;
    double **f = atom->f;
    int *type = atom->type; 
    ...
    auto polarizability_tensor = results.at("polarizability_p").to(torch::kDouble).cpu().reshape({-1});
    auto polarizability = polarizability_tensor.data_ptr<double>();
    vector[0] = polarizability[0];
    vector[1] = polarizability[4];
    vector[2] = polarizability[8];
    vector[3] = (polarizability[5] + polarizability[7]) / 2;
    vector[4] = (polarizability[2] + polarizability[6]) / 2;
    vector[5] = (polarizability[1] + polarizability[3]) / 2;
}
